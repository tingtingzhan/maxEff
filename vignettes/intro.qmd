...
The `S3` method dispatch `print.add_numeric()` displays the `call`s to the selected `numeric` predictors. Developers may pass these calls to parameter `ee` of the `S3` method dispatch `spatstat.geom::with.hyperframe()` for further analysis.
...
The S3 method dispatch `predict.add_numeric()` applies the starting model structure in *`m0`*, as well as the additional `numeric` predictors selected by *`a0`*, on the test set *`s1`*. The authors categorize this functionality under the `S3` generic `stats::predict()`, instead of `stats::update()`, to emphasize that this "update" is on a `newdata`, even that we use the workhorse `S3` method dispatch `stats:::update.default()`.
...
-   dichotomizes each "slice" of the `numeric`-hypercolumn *`logKi67.quantile`* into a `logical` variable, see Appendix @sec-node1;
-   remove the `duplicated` `logical` variables
-   `update`s the starting model *`m0`* with each one of the additional `logical` predictors, respectively;
-   returns an `'add_dummy'` object, which inherits from class `'add_'`. This is a `listof` objects with internal class `add_dummy_`, see Appendix @sec-add_dummy_.
...
The `S3` method dispatch `print.add_dummy()` displays the `call`s to the selected `logical` predictors. Developers may pass these calls to parameter `ee` of the `S3` method dispatch `spatstat.geom::with.hyperframe()` for further analysis.
...
```{r}
#| code-fold: true
#| code-summary: 'Developers: retrieve the 1st selected `logical` predictor'
s0 |> 
  with(ee = str2lang(labels(b0[[1L]]))) |> # ?spatstat.geom::with.hyperframe
  table()
```

```{r}
#| code-fold: true
#| code-summary: 'Developers: retrieve the 2nd selected `logical` predictor'
s0 |> 
  with(ee = str2lang(labels(b0[[2L]]))) |> # ?spatstat.geom::with.hyperframe
  table()
```
...
Similar to @sec-add_dummy, the `S3` method dispatch `print.add_dummy()` displays the `call`s to the selected `logical` predictors. Developers may pass these calls to parameter `ee` of the `S3` method dispatch `spatstat.geom::with.hyperframe()` for further analysis.
...
```{r}
#| code-fold: true
#| code-summary: 'Developers: retrieve the 1st selected `logical` predictor via repeated partitions'
s0 |> 
  with(ee = str2lang(labels(c0[[1L]]))) |> # ?spatstat.geom::with.hyperframe
  table()
```

```{r}
#| code-fold: true
#| code-summary: 'Developers: retrieve the 2nd selected `logical` predictor via repeated partitions'
s0 |> 
  with(ee = str2lang(labels(c0[[2L]]))) |> # ?spatstat.geom::with.hyperframe
  table()
```

Similar to @sec-add_dummy, the S3 method generic `predict.add_dummy()` uses model *`c0`* on the test set *`s1`*.
...
Data example `rpart::stagec` contains 146 patients with [Stage C prostate cancer](https://www.cancer.org/cancer/types/prostate-cancer/detection-diagnosis-staging/staging.html).
...
We create a recursive partitioning model of the numeric variable *`age`* with the endpoint of progression-free survival in the training set *`stagec0`*. In the following call to function `rpart::rpart()`,
...
Function `node1()` creates a **dichotomizing rule** of the numeric variable *`age`* based on the first node in the tree *`rp0`*. Function `node1()` returns an object of class `'node1'`, which is also a `closure` that consists of a `function` and the `environment` in which it was created.
...
The `S3` method dispatch `base::print.function()` displays the dichomizing rule *`fn0`* and its `environment`,

```{r}
fn0
```

The `name` of the numeric variable, e.g., *`age`*, is stored as the `formals` argument of parameter *`newx`*. This is an advanced R programming trick.  The `eval`uation of the dichotomizing rule discovers the variable *`age`* in 

-   the global environment `.GlobalEnv`
-   the enclosure `environment` of the dichotomizing rule, if its `parent.env` is a `namespace` or the `.GlobalEnv`

```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: `.GlobalEnv`'
age = stagec1$age
fn0()
rm(age)
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: *`environment(fn0)`*, child of package `namespace`'
(ev = environment(fn0))
parent.env(ev)
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: `new.env()`, child of `.GlobalEnv`'
ev_a = new.env()
parent.env(ev_a)
assign(x = 'age', value = stagec1$age, envir = ev_a)
ls(envir = ev_a)
fn0_a = fn0
environment(fn0_a) = ev_a
fn0_a()
rm(ev_a, fn0_a)
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced fact: be careful with `as.environment()`, child of `emptyenv()` !'
ev_b = stagec1 |>
  as.environment()
parent.env(ev_b)
fn0_b = fn0
environment(fn0_b) = ev_b
tryCatch(fn0_b(), error = identity)
rm(ev_b, fn0_b)
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: `list2env(., parent = .GlobalEnv)`'
ev_c = stagec1 |> 
  list2env(parent = .GlobalEnv) 
parent.env(ev_c)
fn0_c = fn0
environment(fn0_c) = ev_c
fn0_c()
rm(ev_c, fn0_c)
```
...
The internal class `'add_numeric_'` inherits from class `'call'`, with additional `attributes`
...
The `S3` method dispatch `base::print.default()` displays each regression model with additional `numeric` predictor.

```{r}
#| code-fold: true
#| code-summary: 'Training models *`a0`*: 1st element'
a0[[1L]]
```

```{r}
#| code-fold: true
#| code-summary: 'Training models *`a0`*: 2nd element'
a0[[2L]]
```
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`a1`*: 1st element'
a11 = a0[[1L]] |> 
  predict(newdata = s1)
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`a1`*: 2nd element'
a12 = a0[[2L]] |> 
  predict(newdata = s1)
...
The internal class `'add_dummy_'` inherits from class `'node1'` (see Appendix @sec-node1), with additional `attributes`
...
The `S3` method dispatch `base::print.default()` displays each regression model with additional `logical` predictor.

```{r}
#| code-fold: true
#| code-summary: 'Training models *`b0`* in training set *`s0`*: 1st element'
b0[[1L]]
```

```{r}
#| code-fold: true
#| code-summary: 'Training models *`b0`* in training set *`s0`*: 2nd element'
b0[[2L]]
```

```{r}
#| code-fold: true
#| code-summary: 'Training models *`c0`* in test-subset of training set *`s0`*: 1st element'
c0[[1L]]
```

```{r}
#| code-fold: true
#| code-summary: 'Training models *`c0`* in test-subset of training set *`s0`*: 2nd element'
c0[[2L]]
```
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`b1`*: 1st element'
b11 = b0[[1L]] |> 
  predict(newdata = s1)
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`b1`*: 2nd element'
b12 = b0[[2L]] |> 
  predict(newdata = s1)
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`c1`*: 1st element'
c11 = c0[[1L]] |> 
  predict(newdata = s1)
...
```{r}
#| code-fold: true
#| code-summary: 'Predicted models *`c1`*: 2nd element'
c12 = c0[[2L]] |> 
  predict(newdata = s1)
