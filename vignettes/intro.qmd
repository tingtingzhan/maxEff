...
-   **`groupedHyperframe`** (dev v`r packageVersion('groupedHyperframe')`), for data preparation
...
```{r}
#| warning: false
m = coxph(PFS ~ Tstage, data = s0)
```

```{r}
#| code-fold: true
#| code-summary: Starting `coxph` model *`m`*
summary(m)
```
...
1.  Function `add_numeric()` analyzes the `numeric`-hypercolumn *`logKi67.quantile`*, considers each `slice` as a `numeric` predictor, and `update`s the starting model *`m`* with each one of them respectively. Function `add_numeric()` returns an `add_numeric` object, which is a `listof` objects with an internal class `'add_numeric_'`.

2.  The `S3` method dispatch `sort_by.add_()` sorts the `add_numeric` object by the `abs`olute value of the regression coefficient (i.e., effect size) of the additioanal numeric predictor.

3.  The `S3` method dispatch `utils:::head.default()` chooses the top `n` element from the object returned from the previous step.

The `S3` method dispatch `print.add_()`
...
Function `slices()` is an internal tool to extract the "slices" from a `listof` of equi-`length` `vector`s.

Function `spatstat.geom::with.hyperframe()` provides an elegant and idiot-proof user interface for this purpose, but it is too slow in a batch process.  

```{r}
z1 = s0 |>
  spatstat.geom::with.hyperframe(expr = logKi67.quantile['0.98'])
```
