...
-   `update`s the starting model *`m`* with each one of the additional `numeric` predictors, respectively;
-   returns an `'add_numeric'` object, which is a `listof` objects with internal class `'add_numeric_'`. The `'add_numeric'` inherits from class `'add_'`.

Second, the `S3` method dispatch `sort_by.add_()` 

-   sorts the input `'add_numeric'` object by the `abs`olute values of the regression coefficients (i.e., effect size) of the additional `numeric` predictors;
-   returns an `'add_numeric'` object

Third, the `S3` method dispatch `utils:::head.default()` chooses the top `n` element from the input `'add_numeric'` object.
...
```{r}
Ki67q |> 
  with(ee = formals(m2[[1L]])$newx) |> # ?spatstat.geom::with.hyperframe
  m2[[1L]]() |>
  head(n = 20L)
```
...
```{r}
Ki67q |> 
  with(ee = formals(m2[[2L]])$newx) |> # ?spatstat.geom::with.hyperframe
  m2[[2L]]() |>
  head(n = 20L)
```
...
We create a recursive partitioning model of the numeric variable *`age`* with the endpoint of progression-free survival among the first 140 patients (training set).  In the following call to function `rpart::rpart()`,
...
The `S3` method dispatch `base::print.function()` displays the dichomizing rule *`fn1`*,
