...
```{r}
#| code-fold: true
#| code-summary: 'Recursive partitioning tree *`rp0`* from package **`rpart`**'
rp0
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: *`environment(D0)`*, child of package `namespace`'
(ev = environment(D0))
parent.env(ev)
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: `new.env(parent = .GlobalEnv)`'
ev_a = new.env(parent = .GlobalEnv)
parent.env(ev_a)
assign(x = 'age', value = stagec1$age, envir = ev_a)
ls(envir = ev_a)
D0a = D0
environment(D0a) = ev_a
D0a()
rm(ev_a, D0a)
```
...
parent.env(ev_b)
D0b = D0
environment(D0b) = ev_b
tryCatch(D0b(), error = identity)
rm(ev_b, D0b)
```
...
The `S3` method dispatch `predict.node1()` dichotomize the `numeric` variable *`age`* in the test set *`stagec1`*, using the dichotomizing rule *`D0`* determined by the training set *`stagec0`*.
...
```{r}
#| code-fold: true
#| code-summary: 'Subject to change: get `numeric` cutoff value'
D0 |> 
  get_cutoff()
```
