...
The S3 method dispatch `predict.add_numeric()` applies the starting model structure in *`m0`*, as well as the additional `numeric` predictors selected by *`a0`*, on the test set *`s1`*. The authors categorize this functionality under the `S3` generic `stats::predict()`, instead of `stats::update()`, to emphasize that this "update" is on a `newdata`, even that the workhorse function is the `S3` method dispatch `stats:::update.default()`.
...
Developers are encouraged to learn more about advanced usage of the class `'add_numeric_'` in Appendix @sec-add_numeric_.
...
Developers are encouraged to learn more about advanced usage of the class `add_dummy_` in Appendix @sec-add_dummy_.
...
Developers may obtain the selected `numeric` predictors by passing these `call`s to parameter `ee` of the `S3` method dispatch `spatstat.geom::with.hyperframe()` for further analysis.

```{r}
#| code-fold: true
#| code-summary: '1st selected `numeric` predictor'
s0 |>
...
```{r}
#| code-fold: true
#| code-summary: '2nd selected `numeric` predictor'
s0 |>
...
```{r}
#| code-fold: true
#| code-summary: '1st selected `logical` predictor'
b0[[1L]] |> 
...
```{r}
#| code-fold: true
#| code-summary: '2nd selected `logical` predictor'
b0[[2L]] |> 
  predict.node1(newdata = s0) |>
  table() |> 
  addmargins() 
...
```{r}
#| code-fold: true
#| code-summary: '1st selected `logical` predictor via repeated partitions'
c0[[1L]] |>
  predict.node1(newdata = s0) |>
  table() |> 
  addmargins()
...
```{r}
#| code-fold: true
#| code-summary: '2nd selected `logical` predictor via repeated partitions'
c0[[2L]] |>
  predict.node1(newdata = s0) |>
  table() |> 
  addmargins()
