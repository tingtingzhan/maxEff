...
Function `node1()` dichotomizes a single `numeric` or `factor` `vector` based on the first node of a recursive partitioning and regression tree `rpart.object` from package **`rpart`** [@rpart]. In this section, we illustrate
...
### `numeric` Predictor {#sec-node1_numeric}
...
Function `node1()` creates a dichotomizing rule *`D0b`* for the `factor` variable *`ploidy`* based on the first node in the recursive partitioning tree *`rp0b`*, in a similar procedure as outlined in Appendix @sec-node1_numeric.
