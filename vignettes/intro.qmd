...
Function `node1()` creates a **dichotomizing rule** of the numeric variable *`age`* based on the first node in the tree *`rp0`*. Function `node1()` returns an object of class `'node1'`, which `inherits` from the class `'function'`.
...
The `name` of the numeric variable, e.g., *`age`*, is stored as the `formals` argument of parameter *`newx`*. This is an advanced R programming trick; `eval`uating *`fn0()`* discovers variable *`age`* in `.GlobalEnv` or `environment(fn0)`, but not anywhere else!

```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: put variable *`age`* in `.GlobalEnv`'
age = stagec1$age
fn0()
rm(age)
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: put variable *`age`* in *`environment(fn0)`*'
(ev = environment(fn0))
ls(envir = ev)
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced trick: put variable *`age`* in `new.env()`, **and** make it the `environment` of the dichotomizing rule '
(ev_a = new.env())
...
fn0_a()
tryCatch(eval(fn0(), envir = ev_a), error = identity)
tryCatch(eval(fn0(), enclos = ev_a), error = identity)
rm(list = ls(envir = ev_a), envir = ev_a)
rm(ev_a)
rm(fn0_a)
```
...
```{r}
#| code-fold: true
#| code-summary: 'Advanced fact: be careful with `base::as.environment()`!'
(ev_b = as.environment(list(age = stagec1$age)))
ls(envir = ev_b)
fn0_b = fn0
environment(fn0_b) = ev_b
tryCatch(fn0_b(), error = identity) # um.. why?
rm(list = ls(envir = ev_b), envir = ev_b)
rm(ev_b)
rm(fn0_b)
```
