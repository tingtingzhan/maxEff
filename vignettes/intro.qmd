...
The S3 method dispatch `predict.add_numeric()` applies the starting model structure in *`m0`*, as well as the additional `numeric` predictors selected by *`a0`*, on the test set *`s1`*. The authors categorize this functionality under the `S3` generic `stats::predict()`, instead of `stats::update()`, to emphasize that this "update" is on a `newdata`, even that we use the workhorse function `stats:::update.default()`.
...
The `S3` method dispatch `print.add_dummy()` displays the `call`s to the selected `logical` predictors. Developers may pass these calls to parameter `ee` of function `spatstat.geom::with.hyperframe()` for further analysis.
...
Similar to @sec-add_dummy, the `S3` method dispatch `print.add_dummy()` displays the `call`s to the selected `logical` predictors. Developers may pass these calls to parameter `ee` of function `spatstat.geom::with.hyperframe()` for further analysis.
