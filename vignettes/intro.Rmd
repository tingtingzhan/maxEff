...
Candidate of additional predictors are stored in a `numeric`-`hypercolumn` *`Ki67.quantile`*.
Users are encouraged to learn more about functions `grouped_ppp()` and `aggregate_quantile()` from the **Introduction** vignette of package [**`groupedHyperframe`**](https://CRAN.R-project.org/package=groupedHyperframe).
