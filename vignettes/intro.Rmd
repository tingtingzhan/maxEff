...
This vignette documents three methods of selecting one from many `numeric` predictors for a regression model, to ensure that the additional predictor has the maximum effect size.   The three methods are implemented in functions `add_num()`, `add_dummy()` and `add_dummy_partition()`.
...
c(
  ...
  '`statusPartition`', 'Stratified partition by survival status', '`maxEff::statusPartition`, `caret::createDataPartition`', 
...
## `add_dummy_partition()`: via Repeated Stratified Partitions

Function `add_dummy_partition()` partitions each additional `numeric` predictor into a `logical` variable in the following steps.

1. Generate multiple, i.e., repeated, `statusPartition`s.
2. For each `statusPartition`, create a dichotomizing rule (via function `node1()`) on the training set. Apply this dichotomizing rule on the test set and obtain the estimated regression coefficient (i.e., effect size) of the additional `logical` predictor.
3. Among all `statusPartition`s, select the one with median effect size of the additional `logical` predictor.

Function `add_dummy_partition()` also returns an `'add_dummy'` object.

```{r}
set.seed(14837); m3 = m |> 
  add_dummy_partition(~ Ki67.quantile, times = 20L, mc.cores = 1L) |>
