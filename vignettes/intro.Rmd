...
Function `stratifiedPartition()` performs stratified partition. This is useful as we do not want to end up with a partition in which either of the training or test set having all-`TRUE` or all-`FALSE` responses for logistic regression model, or having all-censored responses for Cox regression model.
...
Function `add_dummy()` partitions each additional `numeric` predictor into a `logical` variable base on the first node of recursive partitioning (via function `node1()`) , and update the starting model with each `logical` additional predictor. Function `add_dummy()` returns an `'add_dummy'` object, which is a `listof` objects with class `'node1'`.
